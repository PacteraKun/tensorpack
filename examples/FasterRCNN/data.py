...
def get_rpn_anchor_input(im, boxes, is_crowd):
    ...
    featuremap_boxes = np.zeros(
        (anchorH * anchorW * cfg.RPN.NUM_ANCHOR, 4), dtype='float32')
...
def get_sniper_rpn_anchor_input(im, boxes, is_crowd, gt_invalid):
    """
    Args:
        im: an image
        boxes: nx4, floatbox, gt. shoudn't be changed
    ...
    def iou(A, B):
        top_left = [max(A[0], B[0]), max(A[1], B[1])]
        bottom_right = [min(A[2], B[2]), min(A[3], B[3])]
        intersection = (bottom_right[1] - top_left[1]) * (
...
def get_multilevel_rpn_anchor_input_ignore(im, boxes, is_crowd, ignore_list):
    """
    Args:
        im: an image
        boxes: nx4, floatbox, gt. shoudn't be changed
    ...
    inside_ind, inside_anchors = filter_boxes_inside_shape(
        all_anchors_flatten, im.shape[:2])
    anchor_labels, anchor_gt_boxes = get_anchor_labels_ignore(
        inside_anchors, boxes[is_crowd == 0], boxes[is_crowd == 1],
        ignore_list)
...
def get_train_dataflow_ignore(ignore_path):
    """
    Return a training dataflow. Each datapoint consists of the following:
    ...
    ignore = None
    if ignore_path is not None and os.path.isfile(ignore_path):
        with open(ignore_path, 'r') as ignore_file:
            ignore_lines = ignore_file.readlines()
            ignore = convert_ignores_to_lists(ignore_lines)
