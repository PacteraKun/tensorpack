...
def sample_fast_rcnn_targets(boxes, gt_boxes, gt_labels):
    ...
    all_indices = tf.concat(
        [fg_inds, bg_inds], axis=0)  # indices w.r.t all n+m proposal boxes
...
# def sample_sniper_targets(boxes, gt_boxes, gt_labels, scale_index):
...
def fastrcnn_losses(labels, label_logits, fg_boxes, fg_box_logits):
    ...
    indices = tf.stack(
        [tf.range(num_fg), tf.to_int32(fg_labels)], axis=1)  # #fgx2
    ...
    with tf.name_scope('label_metrics'), tf.device('/cpu:0'):
        ...
        num_zero = tf.reduce_sum(
            tf.to_int32(tf.equal(fg_label_pred, 0)), name='num_zero')
...
def fastrcnn_predictions(boxes, probs):
    ...
    masks = tf.map_fn(
        f, (probs, boxes), dtype=tf.bool, parallel_iterations=10)  # #cat x N
    ...
    filtered_selection = tf.reverse(
        filtered_selection, axis=[1], name='filtered_indices')
