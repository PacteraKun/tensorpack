...
import model_frcnn
import model_mrcnn
from model_frcnn import (sample_fast_rcnn_targets, sample_sniper_targets,
                         fastrcnn_outputs, fastrcnn_losses,
...
from model_box import (clip_boxes, decode_bbox_target, encode_bbox_target,
                       crop_and_resize, roi_align, RPNAnchors)

from data import (get_train_dataflow, get_eval_dataflow, get_all_anchors,
                  get_all_anchors_fpn, get_sniper_train_dataflow)
...
class DetectionModel(ModelDesc):
    ...
    def fastrcnn_training(self, image, rcnn_labels, fg_rcnn_boxes,
                          gt_boxes_per_fg, rcnn_label_logits,
                          fg_rcnn_box_logits):
        """
        Args:
            image (NCHW):
        ...
        with tf.name_scope('fg_sample_patch_viz'):
            ...
            fg_sampled_patches = tf.reverse(
                fg_sampled_patches, axis=[-1])  # BGR->RGB
            tf.summary.image('viz', fg_sampled_patches, max_outputs=30)

        encoded_boxes = encode_bbox_target(
            gt_boxes_per_fg, fg_rcnn_boxes) * tf.constant(
                cfg.FRCNN.BBOX_REG_WEIGHTS, dtype=tf.float32)
        fastrcnn_label_loss, fastrcnn_box_loss = fastrcnn_losses(
            rcnn_labels, rcnn_label_logits, encoded_boxes, fg_rcnn_box_logits)
    ...
    def fastrcnn_inference(self, image_shape2d, rcnn_boxes, rcnn_label_logits,
        ...
        anchors = tf.tile(
            tf.expand_dims(rcnn_boxes, 1),
            [1, cfg.DATA.NUM_CATEGORY, 1])  # #proposal x #Cat x 4
        decoded_boxes = decode_bbox_target(
            rcnn_box_logits / tf.constant(
                cfg.FRCNN.BBOX_REG_WEIGHTS, dtype=tf.float32), anchors)
...
class ResNetC4Model(DetectionModel):
    ...
    def build_graph(self, *inputs):
        ...
        if is_training:
            ...
            #     proposal_boxes, gt_boxes, gt_labels, image['scale_index'])
...
class ResNetSniperModel(DetectionModel):
    def inputs(self):
        ret = [
            tf.placeholder(tf.float32, (None, None, 3), 'image'),
            tf.placeholder(tf.int32, (None, None, cfg.RPN.NUM_ANCHOR),
            ...
            tf.placeholder(tf.float32, (None, 4), 'gt_boxes'),
            tf.placeholder(tf.int64, (None, ), 'gt_labels'),
            # tf.placeholder(tf.int32, (None, ),  name='scale_index')
        ]  # all > 0
        if cfg.MODE_MASK:
            ret.append(
                tf.placeholder(tf.uint8, (None, None, None),
...
def visualize(model, model_path, nr_visualize=100, output_dir='output'):
    ...
    pred = OfflinePredictor(
        PredictConfig(
            ...
            output_names=[
                'generate_{}_proposals/boxes'.format('fpn' if cfg.MODE_FPN else
                                                     'rpn'),
                'generate_{}_proposals/probs'.format('fpn' if cfg.MODE_FPN else
                                                     'rpn'),
                'fastrcnn_all_probs',
                'final_boxes',
                'final_probs',
    ...
    with tqdm.tqdm(total=nr_visualize) as pbar:
        for idx, dp in itertools.islice(
                enumerate(df.get_data()), nr_visualize):
            img = dp[0]
            if cfg.MODE_MASK:
...
class EvalCallback(Callback):
    ...
    def _before_train(self):
        ...
        self.epochs_to_eval = set(
            [interval * k for k in range(1, EVAL_TIMES + 1)])
        self.epochs_to_eval.add(self.trainer.max_epoch)
        logger.info("[EvalCallback] Will evaluate at epoch " +
                    str(sorted(self.epochs_to_eval)))

    def _eval(self):
        all_results = eval_coco(self.df,
                                lambda img: detect_one_image(img, self.pred))
...
if __name__ == '__main__':
    ...
    if args.visualize or args.evaluate or args.predict:
        ...
        warmup_schedule = [(0, cfg.TRAIN.BASE_LR / 3),
                           (cfg.TRAIN.WARMUP * factor, cfg.TRAIN.BASE_LR)]
        warmup_end_epoch = cfg.TRAIN.WARMUP * factor * 1. / stepnum
        lr_schedule = [(int(np.ceil(warmup_end_epoch)),
                        warmup_schedule[-1][1])]
        ...
        traincfg = TrainConfig(
            model=MODEL,
            data=QueueInput(get_sniper_train_dataflow()),
            callbacks=callbacks,
