...
def fastrcnn_predictions_box_voting(boxes, probs):
    ...
    refined_boxes, refined_probs, refined_indices = tf.map_fn(f, (probs, boxes), dtype=(tf.float32, tf.float32, tf.int32),
                      infer_shape=False, parallel_iterations=10)     # refined_boxes: #cat x N; refined_indices: (cat_id, box_id)
