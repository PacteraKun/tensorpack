...
__all__ = ['PredictConfig', 'PredictConfig_cascade']
...
class PredictConfig_cascade(object):
    def __init__(self,
                 model=None,
                 inputs_desc=None,
                 tower_func=None,
                 ...
                 input_names=None,
                 output_names_1st=None,
                 output_names_2nd=None,
                 output_names_3rd=None, 
        ...
        if self.input_names is None:
            self.input_names = [k.name for k in self.inputs_desc]
        self.output_names_1st = output_names_1st
        self.output_names_2nd = output_names_2nd
        self.output_names_3rd = output_names_3rd
        assert_type(self.output_names_1st, list)
        assert_type(self.output_names_2nd, list)
        assert_type(self.output_names_3rd, list)
        assert_type(self.input_names, list)
        if len(self.input_names) == 0:
            logger.warn('PredictConfig receives empty "input_names".')
        # assert len(self.input_names), self.input_names
        for v in self.input_names:
            assert_type(v, six.string_types)
        assert len(self.output_names_1st), self.output_names_1st
        assert len(self.output_names_2nd), self.output_names_2nd
        assert len(self.output_names_3rd), self.output_names_3rd
